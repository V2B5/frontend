...
import EditarEstabelecimento from "./EstablishmentEdit";
...
function EstabelecimentoPage() {
  ...
  const fetchAvaliacoes = useCallback(async () => {
    try {
      const response = await api.get(`/avaliacao/establishments/${id}`);
  ...
  const updateFotos = async () => {
    try {
      const response = await api.get(`foto/establishments/${id}`);
      const fotoPaths = response.data.data.map(foto => ({
        id: foto.id,
        url: `${process.env.REACT_APP_API_URL}/uploads/establishments/${foto.foto}`,
      ...
      <Grid item xs={12} sm={10} md={11} lg={10} xl={10}>
        <Box sx={{ padding: 0, paddingTop: 0 }}>
          ...
          <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-start', flexWrap: 'wrap', marginTop: -6, marginBottom: 3 }}>
            {utilizador && (
              <>
                <BotaoUpload tipo="establishments" id={id} fotos={fotos} setFotos={setFotos} idUtilizador={utilizador.id} updateFotos={updateFotos} />
          ...
          <FotoSlider fotos={fotos} descricao={estabelecimento.nome} tipo="establishments" id={id} updateFotos={updateFotos} />
